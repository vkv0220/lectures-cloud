...
def format_with_fstring(data: Metrics):
    ...
    tempstring = f"CPU #1: { data.cpu_data[1] }%, Memory used: { data.memory_used }, Load avg: { data.load_avg:.2f}"
    return f"{tempstring:*^64}"
...
def format_with_format(data: Metrics):
    ...
    s = "[{pid}] CPU #7: {cpu}%, Memory used: {memory_used}, Load avg: {load_avg:.2f}"
    s2 = "{:->64}".format(s.format(pid=data.agent.removeprefix("metric-gatherer."), cpu=data.cpu_data[7], memory_used=data.memory_used, load_avg=data.load_avg))
    return s2
